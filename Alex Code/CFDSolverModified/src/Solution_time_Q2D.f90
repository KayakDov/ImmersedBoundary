...
! ############ Introducing initial values #######################
        ...
        ! GPU SETUP: build all six eigendecompositions once, on the GPU, via
        ! CudaBandedLib instead of Alex's six EVDLap*/EVD_Fi CPU builders.
        ! Deliberately placed AFTER Call Init, not at the original EVDLap*
        ! call site (which ran BEFORE Init): the CPU solvers only USE Ckor
        ! (set by Init to 1.5) at SOLVE time inside EVD_Thomas, called every
        ! step, long after Init has run -- so their build-time position never
        ! depended on Ckor. The GPU solvers bake the Helmholtz shift in at
        ! CONSTRUCTION time (see AlexCudaCompatibility.f90), which DOES need
        ! Ckor already set, so construction must happen after Init. Confirmed
