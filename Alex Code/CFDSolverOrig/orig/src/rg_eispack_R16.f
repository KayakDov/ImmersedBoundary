...
c
      ...
      Real(kind=16) ::  a(nm,n),scale(n)
      Real(kind=16) ::  c,f,g,r,s,b2,radix
      logical noconv
c
...
c     .......... guard against zero c or r due to underflow ..........
         if (c .eq. 0.0Q0 .or. r .eq. 0.0Q0) go to 270
         g = r / radix
         f = 1.0Q0
         s = c + r
...
c
  200 return
      ...
      Real(kind=16) ::  ar,ai,br,bi,cr,ci
c
c     complex division, (cr,ci) = (ar,ai)/(br,bi)
c
      Real(kind=16) ::  s,ars,ais,brs,bis
      s = abs(br) + abs(bi)
      ars = ar/s
      ais = ai/s
      brs = br/s
...
c
  ...
  120    continue
c     .......... end interchange ..........
  130    if (x .eq. 0.0Q0) go to 180
         mp1 = m + 1
c
         do 160 i = mp1, igh
            y = a(i,mm1)
            if (y .eq. 0.0Q0) go to 160
            y = y / x
            a(i,mm1) = y
c
            do 140 j = m, n
...
c
      ...
      Real(kind=16) ::  h(nm,n),wr(n),wi(n)
      Real(kind=16) ::  p,q,r,s,t,w,x,y,zz,norm,tst1,tst2
      logical notlas
c
...
c     .......... two roots found ..........
  280 p = (y - x) / 2.0Q0
      ...
      zz = sqrt(abs(q))
      x = x + t
      if (q .lt. 0.0Q0) go to 320
c     .......... real pair ..........
      zz = p + sign(zz,p)
      wr(na) = x + zz
      wr(en) = wr(na)
      if (zz .ne. 0.0Q0) wr(en) = x - w / zz
      wi(na) = 0.0Q0
      wi(en) = 0.0Q0
      go to 330
c     .......... complex pair ..........
...
c
     ...
     x        igh,itn,its,low,mp2,enm2,ierr
      Real(kind=16) ::  h(nm,n),wr(n),wi(n),z(nm,n)
      Real(kind=16) ::  p,q,r,s,t,w,x,y,ra,sa,vi,vr,zz,norm,tst1,tst2
      logical notlas
c
...
c                vectors of upper triangular form ..........
  340 if (norm .eq. 0.0Q0) go to 1001
...
c
            if (wi(i) .ge. 0.0Q0) go to 630
            zz = w
            s = r
            go to 700
  630       m = i
            if (wi(i) .ne. 0.0Q0) go to 640
            t = w
            if (t .ne. 0.0Q0) go to 635
               tst1 = norm
               t = tst1
  632          t = 0.01d0 * t
               tst2 = norm + t
...
c     .......... overflow control ..........
  680       t = abs(h(i,en))
            if (t .eq. 0.0Q0) go to 700
            tst1 = t
            tst2 = tst1 + 1.0Q0/tst1
...
c                eigenvector matrix is triangular ..........
         ...
         go to 730
  720    call cdiv(0.0Q0,-h(na,en),h(na,na)-p,q,h(na,na),h(na,en))
  730    h(en,na) = 0.0Q0
         h(en,en) = 1.0Q0
         enm2 = na - 1
...
c
  ...
  760       continue
c
            if (wi(i) .ge. 0.0Q0) go to 770
            zz = w
            r = ra
            s = sa
            go to 795
  770       m = i
            if (wi(i) .ne. 0.0Q0) go to 780
...
c     .......... solve complex equations ..........
  780       x = h(i,i+1)
            ...
            vi = (wr(i) - p) * 2.0Q0 * q
            if (vr .ne. 0.0Q0 .or. vi .ne. 0.0Q0) go to 784
...
c
      ...
      Real(kind=16) ::  a(nm,n),wr(n),wi(n),z(nm,n),fv1(n)
      integer iv1(n)
c
