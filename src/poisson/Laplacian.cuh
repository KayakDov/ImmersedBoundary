 ...
 * @param rhs Preallocated device array to store the RHS values.
 * @param stream CUDA stream used for asynchronous execution.
 */
template<typename T>
void laplacianRHS(const BoundaryConfig<T>& boundary,
                       SimpleArray<T>& rhs,
                       cudaStream_t stream);
 ...
 */
template<typename T>
SimpleArray<T> laplacianRHS(const BoundaryConfig<T>& boundary,
                            cudaStream_t stream);
 ...
 */
template<typename T>
BandedMat<T> laplacianLinear(const BoundaryConfig<T>& boundary,
                            cudaStream_t stream);
