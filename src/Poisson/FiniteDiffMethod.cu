 ...
 */
template <typename T>
class FiniteDiffMethod : GridDim {
private:    
    Vec<T> _b;
...
public:
    ...
    */
    FiniteDiffMethod(const Mat<T>& frontBack, const Mat<T>& leftRight, const Mat<T>& topBottom, const Vec<T>& b, Mat<T>& preAlocatedForBandedA, Vec<int32_t>& prealocatedForIndices, cudaStream_t stream):
...
void freeMat(const std::initializer_list<std::reference_wrapper<Mat<T>>> destructMe) {
 ...
 void testPoisson(const size_t dimLength, cudaStream_t& stream) {
    ...
    FiniteDiffMethod<double> solver(frontBack, leftRight, topBottom, b, A, diagonalInds, stream);
