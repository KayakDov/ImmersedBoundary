 ...
 */
template <typename T>
class FiniteDiffMethodPoisson : GridDim {
private:    
    Vec<T> _b;
...
public:
    ...
    */
    FiniteDiffMethodPoisson(const Mat<T>& frontBack, const Mat<T>& leftRight, const Mat<T>& topBottom, const Vec<T>& b, Mat<T>& preAlocatedForBandedA, Vec<int32_t>& prealocatedForIndices, cudaStream_t stream):
        GridDim(frontBack._rows/2, frontBack._cols, topBottom._rows/2),
        _b(b),
        here(0, 0),
        up(1, 1),
        down(2, -1),
...
void freeMat(const std::initializer_list<std::reference_wrapper<Mat<T>>> destructMe) {
 ...
 void testPoisson(const size_t dimLength, cudaStream_t& stream) {
    ...
    FiniteDiffMethodPoisson<double> solver(frontBack, leftRight, topBottom, b, A, diagonalInds, stream);
